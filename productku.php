  ...
  <body>
    <?php include 'header.php'; ?>
      ...
      <script type="text/javascript">
      	...
      	<div class="row pt-3">
          <?php
	$result = mysqli_query($con, "SELECT * FROM kategori ORDER BY nama_kategori ASC");

	if(mysqli_num_rows($result) != 0){
		echo '<ul>';
		while($row = mysqli_fetch_assoc($result)){
			$kat_id = $row['kd_kategori'];
			$result2 = mysqli_query($con, "SELECT * FROM produk WHERE kd_kategori='$kat_id'");

			echo '<li>'.$row['nama_kategori'];
				echo '<ul>';
					while($row2 = mysqli_fetch_assoc($result2)){
						echo '<li>'.$row2['nama_produk'].'</li>';
					}
				echo '</ul>';
			echo '</li>';
		}
